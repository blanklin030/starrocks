...
public class SparkClientLogParser extends Thread {
    ...
    public void run() {
        try (
                BufferedReader outReader = new BufferedReader(new InputStreamReader(sparkLoadAppHandle.getProcess().getInputStream()));
                BufferedWriter fileWriter = new BufferedWriter(new FileWriter(sparkLoadAppHandle.getLogPath()))
        ) {
            String line;
